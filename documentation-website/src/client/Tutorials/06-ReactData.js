...
export default () => (
  <BaseTutorial>
    <h1>Part 6: Rendering Data with React</h1>
    <p>
    ...
    <Section title="Using Data with the Book List" id="book-list">
      ...
      <PrismBlock lang="jsx" data-line="8-14">
...
const BookList = ({ response }) => (
  <div className='book-list'>
    ...
    <div className='books'>
      ...
      <Note>
        If our <IJS>children</IJS> function (passed to the{" "}
        <Cmp>CuriProvider</Cmp>) had just passed the <IJS>params</IJS> instead
    ...
    <Section title="Using Data with the Book" id="book">
      <p>
        Instead of using <IJS>response.params</IJS>, we will switch to using{" "}
      ...
      <PrismBlock lang="jsx" data-line="5-21">
        {`// components/Book.js
import React from 'react';
...
const Book = ({ response }) => {
  ...
  return (
    ...
    <Section title="Review" id="review">
      ...
      <CompleteBranch name="07-nav-react" />
      <CodeSandboxDemo id="github/pshrmn/curi-tutorial/tree/06-nav-react" />
    </Section>
    <Section title="Next" id="next">
      <p>
        ...
        a book. In{" "}
        <Link to="Tutorial" params={{ name: "07-nav-react" }}>
          Part 7: Forms & Navigation
