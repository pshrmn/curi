...
export default () => (
  <BaseTutorial>
    ...
    <p>
      ...
      about what our pages should look like. This tutorial will be rendering our
...
`import { CuriBase } from '@curi/react';`
        ...
        <ol>
          ...
          <li>
            ...
            emitted (and during the initial render) and returns the React element(s)
            that make up your website. It will receive three arguments: the
            new <IJS>response</IJS> object, the navigation's <IJS>action</IJS> and
  ...
  ReactDOM.render((
    ...
    />
        ...
        <p>
          ...
          application is rendering nothing (<IJS>null</IJS>). We'll come back to that
          in a minute, but first we should learn about the other React component that
          we'll be using.
        </p>
      </Subsection>
...
`import { Link } from '@curi/react';`
        ...
        <p>
          The <Cmp>Link</Cmp> component renders anchor (<Cmp>a</Cmp>)
          elements. However, unlike an anchor, we don't actually have
          to write the URI that we want to navigate to (the <IJS>href</IJS>).
          Instead, you use the <IJS>to</IJS> prop to pass the name of the
          route that you want to navigate to.
        </p>
        <PrismBlock lang='jsx'>
          {
...
// <a href='/books/1357'>Some Book</a>`
        ...
        <p>
          If you need to pass any other location properties (<IJS>query</IJS>, <IJS>hash</IJS>,
          or <IJS>state</IJS>), you can provide them using the <IJS>details</IJS> prop.
        </p>
        <PrismBlock lang='jsx'>
          {
...
export default function(response) {
  return <response.body />;
      ...
      <p>
        ...
        <IJS>Home</IJS> component). Unfortunately, there is no way to navigate to
        any of our other pages. In order to do this, we will need to add some{' '}
    ...
    >
      <p>
        ...
        navigation to our application. From this menu, we only need to
        be able to navigate to our "Home", "Contact", "Book List", and
        "Checkout" routes. Navigation to individual books will be done
        from the book list page.
      </p>
      <p>
        We will use a <Cmp>nav</Cmp> element as the parent for our{' '}
        <Cmp>NavLinks</Cmp>. Inside of that is a <Cmp>ul</Cmp> and then
        each of our routes will be <Cmp>Link</Cmp>s wrapped in <Cmp>li</Cmp>s.
      </p>
      <PrismBlock lang='jsx'>
        {
...
export default NavLinks;`
      ...
      <p>
        ...
        <IJS>response.body</IJS> component, we can return a <Cmp>div</Cmp>
        {' '}that wraps both <IJS>response.body</IJS> and our{' '}
        <Cmp>NavLinks</Cmp> component.
      </p>
      <PrismBlock lang='jsx'>
        {
`// render.js
...
export default function(response) {
  ...
  return (
    ...
    >
      <p>
        Our "Book" route is different than all of our other routes because
        the book path includes an <IJS>id</IJS> param. This means that we need
        to actually have "id" values to pass to our <Cmp>Link</Cmp>s. Later on,
...
export default books;`
        ...
        <p>
          In the <Cmp>BookList</Cmp> component we can iterate over
          this list to generate links to our books.
        </p>
        <PrismBlock lang='jsx'>
        {
`// components/BookList.js
...
const BookList = () => (
  <div className='book-list'>
    ...
    >
      <p>
        The <Cmp>Link</Cmp>s above allow us to navigate to our books, but
      ...
      <p>
        The <IJS>params</IJS> object is a property of our response object.
        That means that if we pass our response object as a prop to the{' '}
...
export default function(response) {
  ...
  return (
    <div>
      ...
      <Note>
        There are a number of ways that you can decide to pass props to
        your route components. The one thing to keep in mind is that{' '}
        <em>all</em> of your route components will receive the same set of
        props. You can either be very specific and only pass the props that
        are necessary (e.g. <Cmp>Body params={`{params}`}</Cmp>) or you can
        just pass the entire response object (e.g. <Cmp>Body response={`{response}`}</Cmp>)
        so you don't have to worry about updating this every time one of
        your route components needs another prop from the response.
      </Note>
      <p>
        Next, we just need to update our <Cmp>Book</Cmp> component so that
        it can access its <IJS>params</IJS> prop and figure out which book
