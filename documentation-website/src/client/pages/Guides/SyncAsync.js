...
export default ({ name }) => (
  <BaseGuide>
    ...
    <p>
      When a navigation event is triggered (e.g. clicking a link or button), the
      router will match a route. If the route is synchronous, the response will
      be emitted immediately. If the route is asynchronous, the response will
      not be emitted until the route's async functions have finished. This is
      useful for code-splitting and preloading data for a route.
    </p>

    <p>
      By default, routes are synchronous. If a route has an{" "}
      <IJS>on.initial</IJS> or <IJS>on.every()</IJS> function, it becomes async.
    </p>
  ...
  on: {
    ...
    <Section title="Async Things to Think About" id="think">
      <p>
        ...
        Curi is sync or async. However, there are a couple of things that you
      ...
      <ol>
        <li>
          <p>
            ...
            before rendering. If you don't, then the app will have to know how
            to render when a response is <IJS>null</IJS>.
          </p>
          <p>
            Waiting is done by passing a function that renders the app to{" "}
            <IJS>router.respond()</IJS>. That function will be called once the
