...
export default ({ name }) => (
  <BaseGuide>
    ...
    <p>
      By default, Curi will perform synchronous matching. Responses for any
      routes with async methods (<IJS>on.initial()</IJS> and{" "}
      <IJS>on.every()</IJS>) will be emitted asynchronously.
    </p>
...
const routes = [
  { name: "Home", path: "" },
  {
    ...
    <p>
      ...
      is sync or asynch. However, there are a couple of things that you should
      be aware of when it comes to async matching.
    </p>

    <ol>
      <li>
        <p>
          If you have async routes, you should wait for the initial response
          before rendering. This can be done by using{" "}
          <IJS>router.respond()</IJS> to delay rendering until the initial
          response is ready. This isn't absolutely necessary, but if you do not
          do this, the initial render might be performed with a response that is{" "}
          <IJS>undefined</IJS>
        </p>
        <PrismBlock lang="javascript">
  ...
  ReactDOM.render(...);
      ...
      <li>
        <p>
          ...
          navigation can be interrupted with a new navigation. Curi handles this
          internally, but you might want to update your UI after a link/button
          is clicked to indicate that the next page is loading. You can see an
