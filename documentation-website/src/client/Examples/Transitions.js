...
export default ({ name }) => (
  <BaseExample>
    ...
    >
      <p>
        This example uses <IJS>react-transition-group</IJS> (v1) to animate navigation transitions,
        but it should be relatively straightforward to adapt this for other animation packages (e.g. react-motion).
      </p>

      <p>
        All that this does is to render a <Cmp>CSSTransitionGroup</Cmp> around the response's body.
        The only other thing that you need to do is to set a key on the rendered component, which is necessary
        for <Cmp>CSSTransitionGroup</Cmp> to know which of its children are entering/leaving/staying.
      </p>
