...
export default function MigrateReactRouterv3Guide({ name }) {
  return (
    <BaseGuide>
      <h1>{name}</h1>
      <SideBySide>
        <Explanation>
          <p>
            Curi is mostly conceptually similar to React Router versions 2 and
            3.
          </p>
          <ol>
            ...
            <li>
              ...
              used to specify child routes that build off of the paths from
              their parent routes.
            </li>
          </ol>
          <p>
            Migration from React Router v2/3 to Curi should not require a
            complete reworking of your application, but there are some key
            differences.
          </p>
          <ol>
            <li>
            ...
            <li>
              ...
              Any ancestor routes that also (partially) match are not rendered.
              This is different from React Router, where ancestors of the best
              matched route also render.
            </li>
          </ol>
        </Explanation>
      </SideBySide>
      ...
      <Section title="Routes" id="routes">
        ...
        <Subsection title="With React Router" id="routes-with-react-router">
          <SideBySide>
            <Explanation>
              <p>
                In React Router v2/3, there are two ways to define routes. You
                can either use JavaScript objects or JSX <Cmp>Route</Cmp>s
                (which React Router converts to JavaScript objects).
              </p>
              <p>
                Both styles described above define the same route structure for
                three routes: <IJS>/</IJS>, <IJS>/inbox</IJS>, and{" "}
                <IJS>/inbox/:message</IJS>. Each one of these has a component
                that will be rendered when it matches. The{" "}
                <IJS>/inbox/:message</IJS> route has some methods defined to
                describe its behavior when the route enters, updates, and
                leaves.
              </p>
            </Explanation>
            <CodeBlock lang="jsx">
          ...
          <SideBySide>
            <Explanation>
              ...
              <p>
                ...
                used for interacting with it. For example, to navigate to a
                route, you only have to know its name, not its URL.
              </p>
              <p>
                The biggest difference between the Curi paths and the React
                Router paths is that with Curi, you never include a forward
                slash at the beginning of the path. This means that while the
                root path for React Router is <IJS>'/'</IJS>, the root path for
                Curi is <IJS>''</IJS>.
              </p>
            </Explanation>
            <CodeBlock>
        ...
        path: ':message'
          ...
          <SideBySide>
            <Explanation>
              ...
              <p>
                With Curi, the router creates a "response" object when it
                matches locations. Some of the properties of the response are
                ...
                an object whose properties will be added to the response. For
                this React application, we want a response's <IJS>body</IJS>{" "}
                property to be the React component associated with each route.
              </p>
              <Note>
                Only known properties will be merged onto the response.
              </Note>
            </Explanation>
            <CodeBlock>
    ...
    children: [
      {
        ...
        response: () => {
          ...
          <SideBySide>
            <Explanation>
              <p>
                We are close to replicating our React Router routes, but we
                still need to implement the <IJS>on___</IJS> methods for our <IJS
                >
                  :message
                </IJS>{" "}
              ...
              <p>
                ...
                will receive the previous props, which could be used to
                determine which props changed. The functionality for both{" "}
                <IJS>onEnter</IJS> and <IJS>onChange</IJS> can be covered using
                a <IJS>resolve</IJS> function.
              </p>
              <p>
                There currently is no equivalent to <IJS>onLeave</IJS> with
                Curi. This is mostly because I haven’t seen a compelling need
                for it. It certainly could be implemented, but so far I have not
                found a reason to use that. If you have something you need this
        ...
        <SideBySide>
          <Explanation>
            <p>
              ...
              <Cmp>Router</Cmp>. That either takes the <Cmp>Route</Cmp>{" "}
              components as props or the route objects through its{" "}
              <IJS>routes</IJS> prop. The <Cmp>Router</Cmp> also takes a{" "}
              <IJS>history</IJS> prop, which is either one of the pre-routerured
              objects (<IJS>browserHistory</IJS> or <IJS>hashHistory</IJS>) or
        ...
        <SideBySide>
          <Explanation>
            <p>
              ...
              similar to the <IJS>history</IJS> package used by React Router,
              but has a slight modified API (easier navigation blocking and
              navigation that imitates how anchors work) and more convenient
              location objects (you can use a <IJS>query</IJS> object instead of
              having to manually create a <IJS>search</IJS> string).
            </p>
          </Explanation>
          <CodeBlock>
...
const router = curi(history, routes);`}
      ...
      <Section title="Rendering" id="rendering">
        <SideBySide>
          <Explanation>
            <p>
              We will walk through the rendering differences between React
              Router and Curi by looking at what happens in each when we
              navigate to the URI <IJS>/inbox/test-message-please-ignore</IJS>.
            </p>
          </Explanation>
        </SideBySide>
        <Subsection title="React Router v2/3" id="rendering-react-router">
          <SideBySide>
            <Explanation>
              <p>
                React Router uses the <Cmp>Router</Cmp> component to subscribe
                to location changes. Each time that the location changes, it
                walks over its routes and determines which route(s!) match.
              </p>
              <p>
                React Router starts by rendering the root component. In the
                above router, that is the <Cmp>App</Cmp>. Next, our{" "}
                <IJS>inbox</IJS> route also matches, so React Router also
                renders our <Cmp>Inbox</Cmp> component. Finally, the URI{" "}
                ...
                <IJS>/inbox/:message</IJS>), so <Cmp>Message</Cmp> is rendered
                as well. Each child component is rendered by its parent, so we
                end up with a component tree that looks something like this:
              </p>
              <p>
                ...
                need to know how to render based on what type of match they
                have. For example, <Cmp>Inbox</Cmp> needs to know how to render
                for an exact match (the URI is <IJS>/inbox</IJS>) and for a
                partial match (<IJS>/inbox/test-message-please-ignore</IJS>).
                Also, if the <Cmp>Inbox</Cmp> needs to pass any props to{" "}
                <Cmp>Message</Cmp>, it has to use <IJS>React.cloneElement</IJS>,
          ...
          <SideBySide>
            <Explanation>
              <p>
                ...
                that the location changes. We will do this by creating a root
                Curi component by calling the <IJS>curiProvider()</IJS>{" "}
                function, which comes from the <IJS>@curi/react-dom</IJS>{" "}
                package, and passing it our Curi router. While the name of this
                component is entirely up to you, we will refer to it as the{" "}
                <Cmp>Router</Cmp> here.
              </p>
              <p>
                The <Cmp>Router</Cmp> will setup an observer on the provided
                router so that it can re-render your application whenever there
                is a new <IJS>response</IJS>. The <Cmp>Router</Cmp> expects a
              ...
              <p>
                Above, we added <IJS>response()</IJS> functions to each route.
                The functions set React components as the <IJS>body</IJS>{" "}
                property of responses. We can now use <IJS>response.body</IJS>{" "}
                to render those components.
              </p>
              <p>
                In the React Router section, we had three components that were
                rendered: <Cmp>App</Cmp>,
                <Cmp>Inbox</Cmp>, and <Cmp>Message</Cmp>. With Curi, only the
                most accurately matched route actually matches. That means that
                for the URL <IJS>/inbox/test-message-please-ignore</IJS>, the{" "}
          ...
          <SideBySide>
            <Explanation>
              <p>
                ...
                Props can be passed individually, but passing the whole{" "}
        ...
        <SideBySide>
          <Explanation>
            <p>
              ...
              application. React Router provides a <Cmp>Link</Cmp> component to
              do this, and so does Curi (through the <IJS>@curi/react-dom</IJS>{" "}
...
<Link to='Message' params={{ message }}>Hello</Link>`}
          ...
          <li>
            <SideBySide>
              <Explanation>
                <p>
                  With React Router, any additional location properties are
                  passed to the <Cmp>Link</Cmp> using the <IJS>to</IJS> object.
                  With Curi, these properties are passed using the prop name (<IJS
                  >
                    hash
                  </IJS>, <IJS>query</IJS> &amp;
                  <IJS>state</IJS>).
...
<Link to='Inbox' hash='test'>Inbox</Link>`}
          ...
          <li>
            <SideBySide>
              <Explanation>
                <p>
                  Active detection with Curi uses an <Cmp>Active</Cmp>{" "}
                  component. <Cmp>Active</Cmp>'s <IJS>children</IJS> prop is a
                  render-invoked function that receives a boolean{" "}
                  <IJS>true</IJS> when the named route is active and{" "}
                  <IJS>false</IJS> when it is not. You can also pass{" "}
                  <IJS>{`partial=\{true\}`}</IJS> to let partial matches
    ...
    >Home</Link>
      ...
      >
        <SideBySide>
          <Explanation>
            <p>
              React Router provides a <IJS>withRouter</IJS> higher-order
              component that will inject router props into the wrapped
              component.
            </p>
            <p>
