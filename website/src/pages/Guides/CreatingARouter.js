...
export default function CreatingARouterGuide({ name }) {
  return (
    <BaseGuide>
      <h1>{name}</h1>
        ...
        <ol>
          <li>
            <SideBySide>
              <Explanation>
                <p>
                  The <IJS>browser</IJS> history is used for applications
                  running in a browser. If you use the <IJS>browser</IJS>{" "}
                  history, your application should be hosted on a server that
            ...
            <SideBySide>
              <Explanation>
                <p>
                  ...
                  that can only handle requests for files that exist (static
                  file servers).
                </p>
              </Explanation>
              <CodeBlock>
...
const inMemoryHistory = InMemory();`}
        ...
        <Subsection title="Locations" id="locations">
          <SideBySide>
            <Explanation>
              <p>
                The <IJS>history</IJS> object will map URLs into location
                objects. Only the <IJS>pathname</IJS>, <IJS>query</IJS>{" "}
                (search), and <IJS>hash</IJS> segments are used; locations
                ignore the domain and protocol segments of a URL.
              </p>
              <p>
                When matching loactions to routes, only the <IJS>pathname</IJS>{" "}
                is used.
              </p>
            </Explanation>
            <CodeBlock>
          ...
          <SideBySide>
            <Explanation>
              <p>
                ...
                but the history object can be configured to automatically parse
                it into an object.
              </p>
              <p>
        ...
        <SideBySide>
          <Explanation>
            <p>
              ...
              application. When the router receives a new location, it matches
              it against the routes to generate a response.
            </p>
            <p>
            ...
            <p>
              Routes can be nested. A child route's <IJS>path</IJS> will build
              on the paths from any ancestor routes.
            </p>
            <p>
        ...
        <SideBySide>
          <Explanation>
            <p>
              ...
              their names. For example, Curi provides a <IJS>pathname</IJS>{" "}
              route interaction. When you want to navigate to a route within
              your application, you specify the name of the route to navigate to
              (and any params for the route) and Curi will create the URL for
              you.
            </p>
          </Explanation>
          <CodeBlock>
        ...
        <SideBySide>
          <Explanation>
            <p>
              ...
              This either happens from in-app navigation (e.g. clicking a link)
              or platform navigation (e.g. clicking the back button or typing
              URL in the address bar and hitting enter).
            </p>
            <p>
              ...
              want to navigate to and any route <IJS>params</IJS>. The
              navigation <IJS>method</IJS> controls how the history changes
              locations, with the default behavior acting like clicking a link.
            </p>
          </Explanation>
          <CodeBlock>
            {`router.navigate({
        ...
        <SideBySide>
          <Explanation>
            <p>
              ...
              response handlers. There are three types of response handlers:
              observers, one time functions, and side effects.
            </p>
            <p>
            ...
            <p>
              ...
              <IJS>router.once()</IJS>. These are response handlers that will
              only be called one time. If a response already exists, then the
              response handler will be called immediately (unless configured not
              to). Otherwise, the one time response handler will be called after
              the next response is emitted.
            </p>
            <p>
              Observers are passed to the router using{" "}
              <IJS>router.observe()</IJS>. Unlike one time functions, these will
              be called for every response emitted by the router (until you tell
              the router to stop calling it). You most likely will not need to
              call this yourself. The different framework implementations (<IJS>
                @curi/react-dom
              </IJS>, <IJS>@curi/react-native</IJS>, <IJS>@curi/vue</IJS>, and{" "}
        ...
        <SideBySide>
          <Explanation>
            <p>
              ...
              <IJS>resolve</IJS> functions), <IJS>router.once()</IJS> should be
              used to delay the initial render until after the initial response
              is ready.
            </p>
          </Explanation>
          <CodeBlock>
        ...
        <SideBySide>
          <Explanation>
            <p>
              ...
              way that Curi interfaces with each of them varies, but they all
              use observers to be notified when there is a new response.
            </p>
          </Explanation>
        </SideBySide>
        <SideBySide>
    ...
    {({ response }) => {
      ...
      return <Body />;
        ...
        <SideBySide>
          <Explanation>
            <p>
              <IJS>@curi/vue</IJS> sets up reactive objects that update when
              there is a new response. <Cmp>component :is</Cmp> can be used to
              render the <IJS>body</IJS> component.
            </p>
            <p>
              The{" "}
              <Link to="Tutorial" params={{ slug: "react-basics" }}>
