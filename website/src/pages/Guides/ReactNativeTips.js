...
export default function ReactNativeTipsGuide({ name }) {
  return (
    <BaseGuide>
      <h1>{name}</h1>
      <SideBySide>
  ...
  <Text>Home</Text>
        ...
        <Subsection title={<IJS>curiProvider</IJS>} id="fn-curiProvider">
          <SideBySide>
            <Explanation>
              <p>
                <IJS>@curi/react-native</IJS> provides a{" "}
                <IJS>curiProvider()</IJS> function that will create the root
                Curi component for an application. That component will listen
                for new responses and re-render your application.
              </p>
            </Explanation>
            <CodeBlock lang="jsx">
