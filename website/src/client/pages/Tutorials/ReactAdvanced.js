    ...
    <BaseTutorial>
      ...
      <Section title="Setup" id="setup">
        <SideBySide>
          <Explanation>
            <p>
              If you did not complete the React basics tutorial, you should
              either clone its{" "}
            ...
            <p>
              ...
              made using <IJS>yarn</IJS>, but if you only have NPM installed,
              you can use the analagous commands.
            </p>
          </Explanation>
          <CodeBlock lang="bash">
...
npm run start`}
      ...
      <Section title="Async Routes" id="async">
        <SideBySide>
          <Explanation>
            <p>
              Curi lets you attach async functions to a route, and when that
              route matches, a response will not be emitted until the async
              functions have completed. The results of the async functions will
              be available in a route's <IJS>response()</IJS> function under the{" "}
              <IJS>resolved</IJS> object.
            </p>
            <Note>
              These async functions are called every time a route matches. If
              you have functions that should re-use the results from previous
              calls, you will probably want to implement some caching into your
              async functions. Curi provides a{" "}
            ...
            <p>
              ...
              <IJS>resolved</IJS> object. If any of the async functions throws
              an error, that error will be available in the{" "}
        ...
        <SideBySide>
          <Explanation>
            <p>
              ...
              route that matches has asynchronous functions, there is no
              response to render until the async code finishes. This means that
              if you attempt to render immediately after creating a router, the{" "}
              <IJS>response</IJS> that will be passed to the <Cmp>Router</Cmp>'s{" "}
              <IJS>children()</IJS> will be <IJS>null</IJS>.
            </p>
            <p>
              There are a few possible ways to handle this situation. The first
              is to delay rendering by placing your <IJS>ReactDOM.render()</IJS>{" "}
              call inside of a <IJS>router.once()</IJS> callback. This will
    ...
    {({ response }) => {
      ...
      return <Body response={response} />;
        ...
        <SideBySide>
          <Explanation>
            <p>
              For more information on async route properties, please refer to
              the{" "}
...
import(/* webpackChunkName: "Test" */ "./components/Test.js")`}
        ...
        <SideBySide>
          <Explanation>
            <p>
              Currently, the <IJS>routes.js</IJS> module imports all of the
              route modules at the top of the file. In order to add code
              splitting, we need to switch to using <IJS>import()</IJS>.
            </p>
            <p>
      ...
      body: () => import(/* webpackChunkName: "Test" */ "./components/Test.js")
        ...
        <Note>
          <SideBySide>
            <Explanation>
              <p>
                <IJS>import()</IJS> resolves a module object, not a component.
                Our components are exported using default exports, so they will
                be available as the module object's <IJS>default</IJS> property.
              </p>
              <p>
        ...
        <SideBySide>
          <Explanation>
            <p>
              ...
              <IJS>response()</IJS> function through the <IJS>error</IJS>{" "}
              property. We won't be incorporating this into the application
              here, but in a real application you probably want to have a
              fallback component to display an error message (especially if you
              have an offline mode with service workers).
            </p>
          </Explanation>
          <CodeBlock>
        ...
        <SideBySide>
          <Explanation>
            <p>
              ...
              imports at the top of the file and use <IJS>import()</IJS> to
...
registerServiceWorker();`}
        ...
        <SideBySide>
          <Explanation>
            <p>
              ...
              multiple bundles. The initial render will be delayed until after
              the code split bundle for the first route has been loaded.
            </p>
          </Explanation>
        </SideBySide>
      </Section>
      <Section title="Preloading Data" id="preloading-data">
        <SideBySide>
          <Explanation>
            <p>
              Preloading data lets you delay navigation until after the data for
              a route has loaded. This can save you from having to render a
              partial page with spinners if the data takes a while to load.
            </p>
            <p>
              While the data is loading, the user will be able to continue
              interacting with the current page. This means that the user can
              also start a new navigation while the current navigation is
              running. When this happens, Curi knows to to cancel the previous
              navigation and perform the new navigation instead.
            </p>
            <p>
            ...
            <p>
              ...
              components. In a real site you would most likely make API calls to
              a REST or GraphQL endpoint, but here we will simulate this with a
              fake API.
            </p>
          </Explanation>
        </SideBySide>
        <Subsection title="The Fake API" id="fake-api">
          ...
          <SideBySide>
            <Explanation>
              <p>
                First, we will create an <IJS>api.js</IJS> module that exports
                the fake API functions.
              </p>
            </Explanation>
          ...
          <SideBySide>
            <Explanation>
              ...
              <p>
                ...
                both, we can use <IJS>Promise.resolve()</IJS> to return a
                Promise, even though we don't really have any asynchronous code
                being run.
              </p>
            </Explanation>
            <CodeBlock>
  ...
  books.find(b => b.id === id)
        ...
        <SideBySide>
          <Explanation>
            ...
            <p>
              The <IJS>Home</IJS> route already has an <IJS>import()</IJS>,
              which we name <IJS>body</IJS>. We will name the async call to load
              the books data <IJS>"books"</IJS>.
            </p>
            <p>
              The <IJS>Book</IJS> route's <IJS>response()</IJS> also needs to be
              updated to attach the books data (<IJS>resolved.books</IJS>) to
              the response.
            </p>
            <p>
              The <IJS>book()</IJS> API call expects to be given the{" "}
              <IJS>id</IJS> number of the book it should return data for. We can
              grab the correct param (<IJS>id</IJS>) from the <IJS>params</IJS>{" "}
              property. However, when params are parsed, they are stored as
              strings. To convert it to a number, we can use the route's{" "}
    ...
    <div>
      ...
      <Section title="Visualizing Loading" id="loading">
        <SideBySide>
          <Explanation>
            <p>
              At this point, we have the same functionality as the basic
              tutorial, but we have added async data loading. The bundle
              importing has real loading times, but the fake API calls resolve
              immediately, which doesn't necessarily reflect real world
              performance.
            </p>
            <p>
              We can update the fake API to delay resolving so that we can take
              a look at some of the <IJS>@curi/react-dom</IJS> components that
              are navigation-aware. The implementation here isn't important, so
              you can just copy+paste the code. The only thing to know is that
              the <IJS>BOOKS()</IJS> function has a one second delay and the{" "}
    ...
    resolve(book);
        ...
        >
          <SideBySide>
            <Explanation>
              <p>
                The <Cmp>Link</Cmp> component can be called with a
                render-invoked <IJS>children()</IJS> function. If you do this,
                the function will be called with a <IJS>navigating</IJS> boolean
                that indicates whether the router is currently navigating to
                that link. This is useful for when you know that there is a long
                (multiple seconds) delay between when the user clicks the link
                and when the navigation will occur.
              </p>
              <p>
                We can update the <Cmp>Link</Cmp>s in the <Cmp>Home</Cmp>{" "}
                component to using render-invoked functions and display a
                loading spinner while we wait for the book data to load.
              </p>
            </Explanation>
            <CodeBlock lang="jsx">
              {`import { Link } from "@curi/react-dom";
          ...
          <SideBySide>
            <Explanation>
              <p>
                ...
                swapped from a React element to a render-invoked function. We
                wrap the contents in a <Cmp>React.Fragment</Cmp> to avoid
                unnecessary DOM elements. In the function, we render a{" "}
                <Cmp>Spinner</Cmp> when the <Cmp>Link</Cmp> is navigating and{" "}
                <IJS>null</IJS> when it is not.
              </p>
              <Note>
                <IJS>react-spinkit</IJS> is highly customizable, but we are
                sticking with the defaults here. <IJS>react-spinkit</IJS> has a
                default one second render delay, which is why the spinner does
      ...
      <Section title="Async Caveats" id="caveats">
        <SideBySide>
          <Explanation>
            ...
            <p>
              The biggest consideration is that there is nothing the frontend
              can do to get the data for the initial render faster. Your
              application's frontend can only fetch data as it discovers it
              needs it. If you are performing server-side rendering, you may
              want to load the initial data on the server and inject it into the
              page's HTML output. The implementation details for this vary
              greatly and are more related to how you store data (e.g.{" "}
            ...
            <p>
              ...
              the data associated with individual components. React Suspense
              will help with this (and Curi will support it once it releases),
              but this is still a ways out. At the very least, I would recommend
              using Curi for code splitting routes. Whether your should hoist
              other data requirements is something that should be determined on
