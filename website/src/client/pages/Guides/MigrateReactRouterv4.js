...
export default function MigrateReactRouterv4Guide({ name }) {
  return (
    <BaseGuide>
      <h1>{name}</h1>
      <SideBySide>
      ...
      <Section title="Routes" id="routes">
        ...
        <Subsection title="With React Router" id="routes-with-react-router">
          <SideBySide>
            <Explanation>
              <p>
                With React Router v4, <Cmp>Route</Cmp>s are defined in
                components. They are usually grouped together under a{" "}
                <Cmp>Switch</Cmp> so that only a single route from a group
                renders. Nested routes are rendered inside of the compnent
                rendered by the parent <Cmp>Route</Cmp>
              </p>
            </Explanation>
            <CodeBlock lang="jsx">
          ...
          <SideBySide>
            <Explanation>
              <p>
                ...
                in an array of "top-level" routes. Nested routes are grouped
                under their parent route's <IJS>children</IJS> property.
              </p>
              ...
              <p>
                ...
                for interacting with it. For example, to navigate to a route,
        ...
        path: ':message'
          ...
          <SideBySide>
            <Explanation>
              ...
              <p>
                Curi routes can have a <IJS>response()</IJS> property, which is
                a function that returns an object of properties to merge onto
                the response that we will be using to render. For this React
    ...
    children: [
      {
        ...
        response: () => {
          ...
          <SideBySide>
            <Explanation>
              <p>
                With React Router v4, a component's lifecycle methods are used
                for loading data, code splitting, and other non-rendering tasks.
                With Curi, routes can have functions that are called when they
                match the new location. These are grouped under the route's{" "}
        ...
        <SideBySide>
          <Explanation>
            <p>
              ...
              <Cmp>Router</Cmp> component. This may be a{" "}
              <Cmp>BrowserRouter</Cmp>, a <Cmp>HashRouter</Cmp>, a{" "}
              <Cmp>MemoryRouter</Cmp>, or a plain <Cmp>Router</Cmp> that you
              pass your own <IJS>history</IJS> instance to. The{" "}
              <Cmp>___Router</Cmp> components create a <IJS>history</IJS>{" "}
        ...
        <SideBySide>
          <Explanation>
            <p>
              ...
              navigate to the URL with the pathname{" "}
          ...
          <SideBySide>
            <Explanation>
              ...
              <p>
                ...
                <IJS>"/"</IJS>. Since it is not, the next <Cmp>Route</Cmp> will
                be checked. The next route, <IJS>"/inbox"</IJS> matches the
                beginning of the pathname{" "}
                <IJS>"/inbox/test-message-please-ignore"</IJS>. It is not an
                exact match, but that route does not do exact matching, so React
                Router will render its component, <Cmp>Inbox</Cmp>.
              </p>

              <p>
                The <Cmp>Inbox</Cmp> has its own <Cmp>Switch</Cmp> to iterate
                over. Its first route only matches <IJS>"/inbox"</IJS> exactly,
                so it moves on to the next route, which has a <IJS>message</IJS>{" "}
                route param. This route matches and stores{" "}
                <IJS>"test-message-please-ignore"</IJS> as{" "}
                <IJS>match.params.message</IJS>. The <Cmp>Message</Cmp>{" "}
                component will then be rendered, which has access to the{" "}
                <IJS>message</IJS> param.
              </p>
            </Explanation>
            <CodeBlock lang="jsx">
              {`ReactDOM.render((
            ...
            <SideBySide>
              <Explanation>
                <p>
                  ...
                  <IJS>{`partial=\{true\}`}</IJS> to partial matches (ancestor
