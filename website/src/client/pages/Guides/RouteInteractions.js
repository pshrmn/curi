...
export default function RouterInteractionsGuide({ name }) {
  return (
    <BaseGuide>
      <h1>{name}</h1>
      <SideBySide>
        <Explanation>
          <p>
            Route interactions let you interact with a registered route using
            its name.
          </p>
          <p>
            A registered route is generally any route that is in the array of
            routes that you used to create your router. However, some
            interactions only register routes that meet some criteria. For
            example, the <IJS>prefetch</IJS> interaction only registers routes
...
const interaction = createMyInteraction();`}
      ...
      <Section title="Adding Interactions" id="adding">
        <SideBySide>
          <Explanation>
            <p>
              Route interactions are provided to the router call as an array
              using the <IJS>route</IJS> property of the options object (the
              third argument).
            </p>
          </Explanation>
          <CodeBlock>
  ...
  route: [createMyInteraction()]
        ...
        <SideBySide>
          <Explanation>
            <p>
              The route interaction will be added to the router's{" "}
              <IJS>route</IJS> property. When you call an interaction, you pass
              the name of the route that you want to interact with.
            </p>
          </Explanation>
          <CodeBlock>
            ...
            <table>
              ...
              <tbody>
                ...
                <tr>
                  <td>register</td>
                  <td>
                    a function to internally store information about routes
                  </td>
                </tr>
                <tr>
                  <td>get</td>
                  <td>
                    ...
                    other arguments) and perform some task using the related
                    route
                  </td>
                </tr>
                <tr>
...
router.route.confirm('Elsewhere'); // false`}
        ...
        <Subsection title="Slightly more advanced" id="Slightly-more-advanced">
          <SideBySide>
            <Explanation>
              <p>
                You might want to write an interaction that uses data from
                parent routes when registering a route. For example, the
                built-in pathname interaction joins a route's path with it
                parent path(s).
              </p>
              ...
              <p>
                ...
                routes, this will be <IJS>undefined</IJS>. For nested routes,
                this is the value returned by the parent route's{" "}
                <IJS>register()</IJS> function.
              </p>
            </Explanation>
            <CodeBlock>
