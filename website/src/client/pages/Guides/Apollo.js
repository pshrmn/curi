...
export default function ApolloGuide({ name }) {
  return (
    <BaseGuide>
      <h1>{name}</h1>
      <SideBySide>
        <Explanation>
          <p>
            <a href="https://apollographql.com">Apollo</a> is a great solution
            for managing an application's data using{" "}
        ...
        <SideBySide>
          <Explanation>
            <p>
              ...
              can create a response without doing any data fetching and let
              Apollo handle that with its <Cmp>Query</Cmp> component.
            </p>
          </Explanation>
          <CodeBlock>
            {`// routes.js
    ...
    response: () => {
      return {
        ...
        <SideBySide>
          <Explanation>
            <p>
              Any location data that a query needs can be taken from the
              response object. The best way to access this from your components
              would be to pass the <IJS>response</IJS> to the components
              rendered in the <Cmp>Router</Cmp>'s <IJS>children</IJS> prop,
              which is a render-invoked function.
            </p>
          </Explanation>
          <CodeBlock lang="jsx">
            {`// index.js
    ...
    {({ loading, error, data }) => {
      ...
      <Section title="Tight Pairing" id="tight-pairing">
        <SideBySide>
          <Explanation>
            <p>
              You can use your Apollo client instance to call queries in a
              route's <IJS>resolve</IJS> functions. <IJS>resolve</IJS> functions
              are expected to return a Promise, which is exactly what{" "}
    ...
    resolve: {
      data({ params }) {
        ...
        <SideBySide>
          <Explanation>
            <p>
              There are two strategies for doing this. Both approaches require
              you to be able to import your Apollo client in the module where
              you define your routes, which is why we created client in its own
              module in the <Link hash="setup">setup</Link> section.
            </p>
          </Explanation>
          <CodeBlock>
            {`// index.js
        ...
        <SideBySide>
          <Explanation>
            <p>
              ...
              attach the data fetched by Apollo directly to a response through
              its <IJS>data</IJS> property.
            </p>
            <p>
    ...
    <p>
      {response.data.verb.definition}
        ...
        <SideBySide>
          <Explanation>
            <p>
              The second approach is to use a <IJS>resolve</IJS> function as a
              way to cache the data, but also use <Cmp>Query</Cmp>. With this
        ...
        <SideBySide>
          <Explanation>
            <p>
              ...
              the query. Because the query has already been executed, Apollo
              will grab the data from its cache instead of re-sending a request
              to your server.
            </p>
          </Explanation>
          <CodeBlock lang="jsx">
