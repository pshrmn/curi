...
export default function RoutesAndResponsesGuide({ name }) {
  return (
    <BaseGuide>
      <h1>{name}</h1>
      <SideBySide>
          ...
          <SideBySide>
            <Explanation>
              <p>
                ...
                response when they are returned by a route's{" "}
              ...
              <table>
                ...
                <tbody>
                  ...
                  <tr>
                    <td>data</td>
                    <td>
                      A place to attach any data you want to the response, such
                      as data loaded in the route's <IJS>resolve</IJS>{" "}
                      functions.
                    </td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    <td>redirectTo</td>
                    <td>
                      An object describing a route that Curi should
          ...
          <SideBySide>
            <Explanation>
              <p>
                ...
                likely always want to use a route's <IJS>response()</IJS>{" "}
                function to attach a <IJS>body</IJS> property to a response. The
                usual pattern is to use a route's <IJS>body</IJS> property to
                describe which component(s) to render when a route matches. This
                can either be a single component for basic layouts or an object
                with a number of components for{" "}
                <Link
                  to="Example"
          ...
          <SideBySide>
            <Explanation>
              <p>
                ...
                </Link>, the router will use it to generate a location object
                that Curi will automatically redirect to.
              </p>
            </Explanation>
            <CodeBlock>
              {`{
          ...
          <SideBySide>
            <Explanation>
              <p>
                ...
                emit redirects, but this also means that you have to render
                using the redirect response. The{" "}
                <IJS>{`{ emitRedirects: false }`}</IJS> option prevents this.
              </p>
            </Explanation>
            <CodeBlock>
      ...
      <Section title="Routes" id="routes">
        <SideBySide>
          <Explanation>
            ...
            <p>
              A route's <IJS>path</IJS> is used to determine if a route matches
              a location. Path strings use{" "}
              <a href="https://github.com/pillarjs/path-to-regexp">
                <IJS>path-to-regexp</IJS>
              </a>{" "}
              formatting, which allows you to define dynamic path parameters
              that a route should match.
            </p>
            <p>
          ...
          <SideBySide>
            <Explanation>
              <p>
                ...
                that a user is authorized to navigate to a route and loading
                data based on the path parameters parsed from the location.
              </p>
              <p>
              ...
              <p>
                A route with <IJS>resolve</IJS> properties is asynchronous,
                which has effects to be aware of. You can read about these in
                the{" "}
              ...
              <p>
                When <IJS>resolve</IJS> functions are called, they will be
                passed an object with the "match" properties of a response.
                These are the matched route's <IJS>name</IJS>, the{" "}
                <IJS>location</IJS>, an object of parsed <IJS>params</IJS>, and
                an array of the names of <IJS>partial</IJS> route matches.
              </p>
            </Explanation>
            <CodeBlock>
              {`{
      ...
      return UserAPI.get(params.id);
        ...
        <Subsection title="The Response Function" id="route-response">
          <SideBySide>
            <Explanation>
              <p>
                Each route can have a <IJS>response()</IJS> function. The role
                of the <IJS>response()</IJS> function is to return an object of
                properties to merge onto the response object that will be
                emitted for the new location.
              </p>
              <p>
              ...
              <p>
                The first is an object of <IJS>resolve</IJS> properties (the
              ...
              <p>
                ...
                if one of the <IJS>resolve</IJS> functions throws an error and
                you don't catch it.
              </p>
            </Explanation>
            <CodeBlock>
          ...
          <SideBySide>
            <Explanation>
              <p>
                ...
                <IJS>pathname</IJS> by walking over the route objects in the
                order that they are defined in the array. If a route has{" "}
          ...
          <SideBySide>
            <Explanation>
              <p>
                ...
                with the pathname <IJS>"/a/red/yellow"</IJS>, the{" "}
                <IJS>Album</IJS> route will only partially match, so Curi will
                move on to the next route, <IJS>Not Found</IJS>, which has a
                catch all <IJS>path</IJS> that matches every pathname. Routes
                can be configured to allow partial matching, but exact matching
          ...
          <Subsection title="No Matching Route" id="catch-all">
            <Warning>
              <SideBySide>
                <Explanation>
                  <p>
                    If none of your routes match a location, Curi will do
                    nothing! You need to set a catch-all route to match these
                    locations yourself. The best way to do this is to add a
                    route to the end of your routes array with a <IJS>path</IJS>{" "}
                    of <IJS>"(.*)"</IJS>, which will match every pathname.
                  </p>
                </Explanation>
                <CodeBlock>
                  {`{
